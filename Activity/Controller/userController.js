...
const user_followingModel = require('../Model/user_followingModel');
...
let createRequest = async(req, res) => {
    try{
        ...
        if(is_public == true){
            ...
            await user_followingModel.addFollowing( { u_id: follower_id, following_id: user_id } );
...
let acceptRequestHandler = async(req, res) => {
    try{
        let user = req.body;
        let user_id = req.body.user_id;
        let follower_id = req.body.follower_id;
